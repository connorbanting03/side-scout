...
def get_value_picks():
    """
    Returns pre-computed best value picks and most consistent players.
    Generated by generate_value_picks.py during nightly prefetch.
    If the cache file is missing, generate it on-the-fly from cached game logs.
    """
    try:
        filepath = os.path.join(CACHE_DIR, 'value_picks.json')
        data = load_cache_persistent(filepath)

        if data is None:
            # Try generating on-the-fly (no NBA API calls — uses cached game logs)
            try:
                from generate_value_picks import generate_value_picks
                data = generate_value_picks()
            except Exception as gen_err:
                print(f"Value picks generation failed: {gen_err}")
                return jsonify({
                    'error': 'not_available',
                    'message': 'Value picks data is not yet available. It will be generated during the next cache refresh.'
                }), 503

        return jsonify(data)
    except Exception as e:
        return jsonify({'error': str(e), 'message': 'Failed to load value picks'}), 500
